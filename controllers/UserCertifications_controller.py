from schemas.UserCertificationSchema import UserCertificationSchema
from schemas.UserCertificationSchema import UserCertificationSchemas

from models.User import User
from models.UserCertification import UserCertificationTable
...
usercertifications = Blueprint('usercertifications', __name__, url_prefix="/usercertifications")

@usercertifications.route("/", methods=["GET"])
def usercertifications_all():
    # Retrieve all workhistorys
    usercertifications = UserCertification.query.all()
    return jsonify(UserCertificationSchemas.dump(usercertifications))


@usercertifications.route("/user/<string:user>", methods=["GET"])
def usercertifications_user(user):
    # Retrieve a particular users workhistorys

    usercertification_object = User.query.get(user)

    if not usercertification_object:
        return abort(401, description="Invalid user")

    usercertifications = UserCertification.query.filter_by(username=user)

    if not usercertifications:
        return abort(404, description="No study histories to return")

    usercertification = UserCertification.query.filter_by(username=user).order_by(UserCertification.date_start.desc()).all()
    return jsonify(UserCertificationSchemas.dump(userstudyhistorys))


@usercertifications.route("/user/<string:user>", methods=["POST"])
@jwt_required
def usercertifications_create(user):
...
@usercertifications.route("/<int:id>", methods=["GET"])
def usercertifications_get(id):
...
@usercertifications.route("/user/<int:id>", methods=["PUT", "PATCH"])
@jwt_required
def usercertifications_update(id):
...
@usercertifications.route("/<int:id>", methods=["DELETE"])
@jwt_required
def usercertifications_delete(id):

    #Delete a study history

    jwt_username = get_jwt_identity()

    UserCertificationObject = UserCertificationTable.query.filter_by(id=id).first()

    if UserCertificationObject is None:
        return abort(401, description=f"There does not exist a studyhistory with id {id}")


    if (jwt_username != UserCertificationObject.username):
        return abort(401, description=f"You are logged in as username: {jwt_username} but the study history id matches to {UserCertificationObject.username} ")

    # Check the user that wants to delete the studyhistory
    userStudyHistory = UserCertificationTable.query.filter_by(id=id, username=jwt_username).first()
